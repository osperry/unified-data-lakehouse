"""
gold_model_premium.py
─────────────────────────────────────────────────────────────────────────────
Exports all gold tables built by dbt to Parquet (ZSTD) and uploads to
Cloudflare R2.  Does NOT rebuild tables — dbt handles that in the
nyc311_silver_gold Dagster asset.

Tables exported (all in main_gold schema, built by dbt):
...
Environment variables (set by Docker via .env):
    WAREHOUSE_PATH         /app/data/nyc311.duckdb
...
WAREHOUSE         = os.getenv("WAREHOUSE_PATH",     "/app/data/nyc311.duckdb")
...
# All gold tables built by dbt — schema is main_gold (dbt default + schema: gold)
GOLD_TABLES = [
    "fct_address_risk_score",
    "fct_anomaly_summary",
    "fct_borough_summary",
...
def export_parquet(con, export_dir: str) -> dict:
    """Export each gold table to a Parquet file. Returns {table_name: local_path}."""
...
def main():
    ...
    # Verify all gold tables exist before exporting
    ...
    for name in GOLD_TABLES:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"Run nyc311_silver_gold (dbt build) first. Error: {e}"
            )
